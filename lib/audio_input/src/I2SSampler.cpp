...
void i2s_reader_task(void *param)
{
   I2SSampler *sampler = (I2SSampler *)param;
    ...
    //    if (xQueueGenericReceive(sampler->m_i2sQueue, &evt, portMAX_DELAY, pdFALSE) == pdPASS)
       // {
          ...
          //  {
                ...
                do
                {
                    ...
                    {
                      printf("%02x",i2sData[i]);
                      if(((i+1)/16)*16 == i) printf("\n");
                    }
                    */
...
void I2SSampler::start(i2s_port_t i2sPort, i2s_config_t &i2sConfig, SampleSink *sample_sink)
{
  ...
  xTaskCreate(i2s_reader_task, "i2s Reader Task", 4096, this, 1, &m_i2s_reader_task_handle);
    }
}

void I2SSampler::stop()
{
  vTaskDelete(m_i2s_reader_task_handle);
    // stop the i2S driver
